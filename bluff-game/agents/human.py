...
class HumanAgent(BaseAgent):
    ...
    def _print_game_state(self, observation: Dict) -> None:
        ...
        if observation["cards_other_agent_played"] > 0:
            print(
                f"Last play: Opponent played {observation['cards_other_agent_played']} cards"
            )
        else:
    ...
    def _get_action_input(self) -> List[int]:
        ...
        while True:
            try:
                ...
                print(
                    "Invalid input. Please enter numbers separated by spaces, or 'c' to challenge"
                )
    ...
    def select_action(self, observation: Dict, mask: List[List[int]]) -> List[int]:
        ...
        while True:
            ...
            if action == self.ACTION_CHALLENGE and not any(
                tuple(m) == tuple(self.ACTION_CHALLENGE) for m in mask
            ):
                ...
                if total_cards == 0:
                    ...
                    for i, (want_play, have) in enumerate(zip(action, hand)):
                        if want_play > have:
                            print(
                                f"You don't have {want_play} {self.RANKS[i]} (you only have {have})"
                            )
                            break
                    else:
                        print(
                            "This play is not valid for the current game state - remember you can only play 0-4 cards total"
                        )
