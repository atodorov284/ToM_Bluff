...
class SecondOrderAgent(BaseAgent):
    ...
    def custom_round(self, number: int):
        ...
        return (
            math.floor(number)
            if number < (math.floor(number) + threshold)
    ...
    def estimate_opponent_cards(self, observation: Dict) -> List[List[float]]:
        ...
        cards = []
        remaining_cards = [
            4 - my_hand[i] for i in range(4)
        ]  # 4 cards per rank initially
        total_remaining = sum(remaining_cards)

        for i in range(4):
            if total_remaining > 0:
                prob = self.custom_round(
                    (remaining_cards[i] / total_remaining) * total_cards
                )
            else:
                prob = 0
            cards.append(prob)
    ...
    def estimate_opponent_observation(
        self, last_action: int, observation: Dict, opponent_cards: List[float]
    ) -> Dict:
    ...
    def estimate_opponent_bluff_probability(
        self, observation: Dict, opponent_cards: List[float]
    ) -> float:
        """
        ...
        opponent_observation = self.estimate_opponent_observation(
            last_action, observation, opponent_cards
        )
    ...
    def wins_interaction(
        self, last_action: int, observation: Dict, opponent_cards: List[float], mask
    ) -> bool:
    ...
    def select_action(self, observation: Dict, mask: List) -> List[int]:
        ...
        if observation["cards_other_agent_played"] > 0:
            bluff_prob = self.estimate_opponent_bluff_probability(
